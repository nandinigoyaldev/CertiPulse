...
app.post('/api/preview-certificate', async (req, res) => {
  try {
    ...
    const pdfBuffer = await generateCertificateBuffer({
      ...
      themeColor: themeColor || '#e05638',
      customBackground: customBgDataUrl || null,
      templatePreset: templatePreset || 'modern',
      layoutSettings: {
        showName: String(showName) !== 'false',
        showQr: String(showQr) !== 'false',
        showEvent: String(showEvent) === 'true',
        showSubtitle: String(showSubtitle) === 'true',
        showBadge: String(showBadge) === 'true',
        showFooter: String(showFooter) === 'true',
        nameY: Number.parseInt(nameY || '230', 10),
        nameSize: Number.parseInt(nameSize || '34', 10),
        eventY: Number.parseInt(eventY || '310', 10),
        eventSize: Number.parseInt(eventSize || '20', 10),
        qrX: Number.parseInt(qrX || '680', 10),
        qrY: Number.parseInt(qrY || '430', 10),
        qrSize: Number.parseInt(qrSize || '80', 10),
        customLayers: Array.isArray(customLayers) ? customLayers : [],
      },
    });
